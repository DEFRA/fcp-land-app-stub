...
const credentials = {
  ...
  sessionId: 'session-page-session-id',
  crn: '1234567890',
...
describe('GET /session', () => {
  ...
  test('should redirect to sign in if unauthenticated', async () => {
    const response = await server.inject({ url: '/session' })
    expect(response.statusCode).toBe(HTTP_STATUS_FOUND)
    expect(response.headers.location).toBe('/auth/sign-in?redirect=/session')
  })
  ...
  test('should render the session view', async () => {
    const response = await server.inject({
      url: '/session',
      auth: { strategy: 'session', credentials }
    })
    expect(response.request.response.source.template).toBe('session')
  })
